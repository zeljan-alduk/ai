...
fastest local-LLM path; Linux and other platforms continue to use
the `openai-compat` adapter against Ollama, vLLM, llama.cpp, or
LM Studio.

> Ollama, vLLM, llama.cpp, and MLX are runtime backends — they're
> the host process the gateway talks to. They are not LLM providers
> in the LLM-agnostic sense; the routing layer above them still
> resolves models by capability class.
...
`draftModel`, `samplerSeed`) live under `providerConfig.extra` and
are translated by the adapter into the snake_case fields
`mlx_lm.server` expects. Other backends ignore these — adding them
does not break LLM-agnosticism.

`MLX_BASE_URL` overrides the base URL at runtime; the API's
`/v1/models` `available` flag flips to `true` once the env var is
set or the default `http://localhost:8081` resolves.

## Eval-harness comparison

Compare quantization tiers head-to-head by registering the same
model under different ids:
...
- The adapter is a thin wrapper over `createOpenAICompatAdapter` —
  ...
  inherited unchanged. MLX-specific behaviour lives entirely
  inside `platform/gateway/src/providers/mlx.ts`.
- Privacy tier `sensitive` is allowed: the model never leaves the
  box.
- The router never branches on `provider === 'mlx'`; routing
  remains capability/privacy/cost based.
