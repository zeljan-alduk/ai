...
import { GitError, createPolicy } from './policy.js';
import { createAldoGitServer } from './server.js';
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  const server = createAldoGitServer({ policy });
...
function fatal(err: unknown): never {
  if (err instanceof GitError) {
    process.stderr.write(`aldo-mcp-git: ${err.code} ${err.message}\n`);
  } else if (err instanceof Error) {
    process.stderr.write(`aldo-mcp-git: ${err.message}\n`);
  } else {
    process.stderr.write(`aldo-mcp-git: ${String(err)}\n`);
  }
  process.exit(2);
}
