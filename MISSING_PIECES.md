...
|---|---|---|---|
| 2026-05-05 | **§13 / item 5.5b — live:network mode wired (env-gated smoke)** | ✅ shipped (operator-invokable) | `apps/api/src/runtime-bootstrap.ts` exports `loadProviderStateForLiveDryRun`. `apps/api/tests/agency-dry-run/healthz-db.ts` adds `runLiveNetworkMode` + typed `LiveNetworkUnavailable`. Production gateway (`createGateway` with resolved provider state) + production tool host (`createMcpToolHost`) replace the stubs. Without creds, throws cleanly; smoke is `describe.skipIf(!ALDO_DRY_RUN_LIVE)` so CI never burns inference. **543/543 apps/api** (542 pass + 1 properly skipped). Operator pre-flight + invocation captured in `agency/dry-runs/2026-05-04-healthz-db.md` §13. **Agency primitive now ships in three forms** (stub / live no-network / live:network). The platform side is done; what remains is a go/no-go on dogfood vs. friendly-first-customer. |
| 2026-05-05 | **§13 / item 5.6 — engine spawn recurses on composite specs** | ✅ shipped (gap closed) | `platform/engine/src/runtime.ts`: new private `spawnCompositeWrapper` helper used by both `runAgent` (depth=0 roots) and `spawn`'s new composite branch (depth=parent+1, nested). Depth tracking added to `runMeta`. `CompositeAgentRun.collectUsage` shim returns `[totalUsage]` so the supervisor adapter treats composite child returns uniformly with leaf returns. `asSupervisorAdapter` branches on `composites` vs `runs` for the right wait/collectUsage closure. **+1 2-level test in `engine-integration.test.ts`** asserting outer→middle→leaf×2 expansion lands 4 runs with correct linkage. **53/53 orchestrator tests; 542/542 apps/api** (was 541; +1 from dry-run rebalance). Live-mode dry-run now lands **6 runs** (was 2): all 5 expected child agents (architect, tech-lead, code-reviewer, security-auditor, backend-engineer) spawn through 3 composite-rollup levels. **5.5b (real provider creds + real MCP tool host) is the last remaining item before a real customer-shaped PR.** Full §12 update in `agency/dry-runs/2026-05-04-healthz-db.md`. |
| 2026-05-05 | **§13 / item 5.5a — live-mode harness (no network) + real engine finding** | ✅ shipped (with discovered gap) | `runDryRun({mode: 'live'})` wires real `PlatformRuntime` + `Supervisor` + `InMemoryRunStore` + stub gateway + stub tool host against the loaded agency YAMLs. **Proves**: engine's `runAgent` dispatches composite specs to the orchestrator, supervisor's `spawnChild → spawn` chain lands children with correct parent/root linkage, `composite.{child_started,child_completed,usage_rollup}` fire and roll up. **Surfaces**: `PlatformRuntime.spawn` does NOT recurse on nested `composite` blocks — it always builds a `LeafAgentRun`, so architect's own composite (architect → tech-lead → engineer + reviewer) is silently skipped. The engine-integration test suite was 1-level only; this dry-run found the multi-level gap without spending a dollar of inference. **+4 live-mode tests, 12/12 dry-run tests green; 537/537 apps/api** stable. New item **5.6** (engine `spawn` recursing on composite specs, ~1–2 d) is the next leveraged chunk. Live post-mortem at `agency/dry-runs/2026-05-05-healthz-db-live.md`; full §11 update in `2026-05-04-healthz-db.md`. |
| 2026-05-05 | **§13 / item 5.4 — driver harness (stub mode)** | ✅ shipped | `apps/api/tests/agency-dry-run/healthz-db.{ts,test.ts,run.mjs}`: `runDryRun({mode})` loads all six brief-touching specs (principal, architect, tech-lead, backend-engineer, code-reviewer, security-auditor) and drives `Supervisor.runComposite` with a `StubRuntimeAdapter` that synthesises plausible per-agent outputs. **Proves**: every YAML loads, composite resolves the principal→architect chain, `composite.*` events fire correctly, cost rolls up, post-mortem renders. **Does NOT prove**: recursive composite expansion (stub treats children as leaves), real model behaviour, real MCP I/O, real PR. **8 new tests + 537/537 apps/api total** green; tsc clean. Live-mode `EngineRuntimeAdapter` is the **one remaining item** (1–2 d). Stub-rendered post-mortem archived at `agency/dry-runs/2026-05-05-healthz-db-stub.md`. |
